...
var sql_register = 'INSERT INTO USER (Id, Password, Name, PhoneNumber, IsAdmin) VALUES(?,?,?,?,?)';
...
router.post('/register', isNotLoggedIn, async (req, res, next) => {
    var params = [];
    const {id, password, name, phoneNumber, isAdmin} = req.body;
    let exUser = "";
    try {
        await conn.query('SELECT * FROM USER WHERE Id = ?', [id], function (err, results, fields) {
            if (err) {
                console.log("ERROR : " + err);
            } else {
                console.log("success");
...
//로그인
router.post('/login', isNotLoggedIn, (req, res, next) => {//req.body.id / req.body.password
...
router.post('/modification', function (req, res, next){
    ...
    console.log(id);
    console.log(name);
    conn.query('UPDATE USER SET Password = ?, Name = ?, PhoneNumber = ? WHERE Id = ?', params, function(err,rows,fields){
